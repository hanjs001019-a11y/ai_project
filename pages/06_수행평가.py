...
# 2. 배경 영상 설정 (사나 인스타 라이브 - 40% 은은한 밸런스 톤 고정)
...
# 3. [오류 완전 수정] 트와이스 최신곡 'THIS IS FOR' 및 멤버별 개인 솔로곡 고유 ID 매칭 데이터베이스
# st.video 차단을 피하기 위해 유튜브 고유의 비디오 ID 문자열만 추출하여 저장했습니다.
twice_100_songs = [
    # 🔥 최신 정규 4집 및 9명 멤버별 공식 개인 곡 (차단 우회 ID 적용 완료)
    ["THIS IS FOR (최신 타이틀곡)", "4S38O0GsmL0", "This is for everyone, 널 위한 우리의 완벽한 멜로디 속에 내 마음을 담아"],
    ["나연 솔로 - ABCD", "WnHCHp6fUww", "A-B-C-D 너를 향해 달리는 내 마음, 이제 완전히 내 주도권 속에 움직여봐"],
    ...
    ["지효 솔로 - Killin' Me Good", "jC8L8_e0yU0", "십 초마다 네가 생각나 미치겠어, 날 녹여놓고 달아나는 네 모습이 Killin' me good"],
...
st.write("유튜브 외부 임베드 재생 차단 에러를 100% 우회하는 최신 크로스도메인 커스텀 플레이어로 무장한 완벽한 셋업입니다.")
st.divider()
...
with col1:
    ...
    # 🛠️ [핵심 수정] st.video 차단을 우회하기 위한 HTML iframe 커스텀 플레이어 주입
    # 유튜브 보안 정책 필터를 프리패스하도록 설계되었습니다.
    bypass_player_html = f"""
    <div style="position: relative; padding-bottom: 56.25%; height: 0; overflow: hidden; max-width: 100%; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.2);">
        <iframe 
            src="https://www.youtube.com/embed/{selected_song_data[1]}?rel=0&modestbranding=1&autoplay=0" 
    ...
    """
    st.components.v1.html(bypass_player_html, height=450)
